...
contract ArcoinTreasury is Ownable, ReentrancyGuard {
    ...
    // Sum of all amount in pending (not-yet-executed) distributions.
    // Prevents queuing more distributions than the treasury can actually pay out.
    uint256 public reservedForDistribution;
    ...
    function queueDistribution(uint256 amount) external onlyOwner {
        if (amount == 0) revert ZeroAmount();
        // Only allow queuing against the *unreserved* balance, so multiple
        // pending distributions can never add up to more than the treasury holds.
        uint256 available = usdc.balanceOf(address(this)) - reservedForDistribution;
        ...
        reservedForDistribution += amount;
    ...
    function executeDistribution(uint256 index) external nonReentrant onlyOwner {
        ...
        reservedForDistribution -= req.amount;
